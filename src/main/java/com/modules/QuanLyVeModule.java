...
public class QuanLyVeModule extends JPanel implements AppModule {
    ...
    private DAO_ChiTietVe daoChiTietVe = new DAO_ChiTietVe();
    ...
    private JPanel buildHeader() {
        ...
        JLabel lblTitle = new JLabel("Qu\u1EA3n l\u00FD v\u00E9 v\u00E0 h\u00F3a \u0111\u01A1n");
    ...
    private JPanel buildStatsRow() {
        ...
        row.add(buildStatCard("T\u1ED4NG H\u00D3A \u0110\u01A0N H\u00D4M NAY", lblStatTongHoaDon, PRIMARY));
        row.add(buildStatCard("DOANH THU V\u00C9 H\u00D4M NAY", lblStatDoanhThu, PRIMARY));
    ...
    private void updateStats() {
        long totalHD = allData.stream()
                .map(v -> v.getHoaDon() != null ? v.getHoaDon().getMaHoaDon() : "")
                .distinct().count();
        lblStatTongHoaDon.setText(String.valueOf(totalHD));

        double totalRevenue = allData.stream()
                .filter(v -> v.getTrangThai() == TrangThaiVe.DA_BAN)
                .mapToDouble(Ve::getGiaTien).sum();
        if (totalRevenue >= 1_000_000) {
            lblStatDoanhThu.setText(String.format("%.1fM VN\u0110", totalRevenue / 1_000_000));
        } else {
            lblStatDoanhThu.setText(FMT_MONEY.format(totalRevenue) + " VN\u0110");
        }
    ...
    private void searchByHoaDon() {
        ...
        filteredData = new ArrayList<>();
        for (Ve ve : allData) {
            if (ve.getHoaDon() != null && ve.getHoaDon().getMaHoaDon().toLowerCase().contains(keyword.toLowerCase())) {
                filteredData.add(ve);
            }
        }
        totalRecords = filteredData.size();
    ...
    private void refreshTable() {
        ...
        // Load chi tiet ve for route info
        List<VeRow> rows = new ArrayList<>();
        for (Ve ve : pageData) {
            String gaDi = "", gaDen = "", khoiHanh = "";
            try {
                List<ChiTietVe> ctvs = daoChiTietVe.findByVe(ve.getMaVe());
                if (!ctvs.isEmpty()) {
                    ChiTietVe first = ctvs.get(0);
                    if (first.getLich() != null && first.getLich().getTuyen() != null) {
                        Tuyen tuyen = first.getLich().getTuyen();
                        gaDi = tuyen.getGaDi() != null ? tuyen.getGaDi().getTenGa() : "";
                        gaDen = tuyen.getGaDen() != null ? tuyen.getGaDen().getTenGa() : "";
                    }
                    if (first.getLich() != null && first.getLich().getThoiGianBatDau() != null) {
                        khoiHanh = first.getLich().getThoiGianBatDau().format(FMT_DATETIME);
                    }
                }
            } catch (Exception ignored) {}
            rows.add(new VeRow(ve, gaDi, gaDen, khoiHanh));
        }
    ...
    private record VeRow(Ve ve, String gaDi, String gaDen, String khoiHanh) {}

    private static class VeTableModel extends AbstractTableModel {
    ...
    private class ActionButtonEditor extends AbstractCellEditor implements TableCellEditor {
        ...
        ActionButtonEditor() {
            ...
            btnDetail.addActionListener(e -> {
                fireEditingStopped();
                VeTableModel.VeRow row = (VeTableModel.VeRow) null;
                var veRow = tableModel.getRowAt(editingRow);
                if (veRow != null) {
                    Ve ve = veRow.ve();
                    JOptionPane.showMessageDialog(QuanLyVeModule.this,
                            "Chi ti\u1EBFt v\u00E9: " + ve.getMaVe() + "\nH\u00E0nh kh\u00E1ch: " + ve.getTenHanhKhach()
                                    + "\nCCCD: " + ve.getCccd() + "\nGi\u00E1: " + FMT_MONEY.format(ve.getGiaTien()) + " VN\u0110",
                            "Chi ti\u1EBFt v\u00E9", JOptionPane.INFORMATION_MESSAGE);
                }
            });
                ...
                if (veRow != null) {
                    ...
                    if (confirm == JOptionPane.YES_OPTION) {
                        ...
                        if (lyDo != null) {
                            ...
                            new DAO_ChiTietVe().huyChiTietVeByMaVe(ve.getMaVe());
                            if (ok) {
                                JOptionPane.showMessageDialog(QuanLyVeModule.this,
    ...
    @Override public String getTitle() { return "Qu\u1EA3n l\u00FD v\u00E9 v\u00E0 h\u00F3a \u0111\u01A1n"; }
