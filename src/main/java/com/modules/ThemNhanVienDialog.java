...
public class ThemNhanVienDialog extends AbstractFormDialog<NhanVien> {
    ...
    public ThemNhanVienDialog(Window owner, Runnable onSaved){super(owner,"Th\u00eam nh\u00e2n vi\u00ean");this.onSaved=onSaved;installStandardLayout(owner);}
    @Override protected String dialogDescription(){return "T\u1ea1o h\u1ed3 s\u01a1 nh\u00e2n vi\u00ean v\u1edbi th\u00f4ng tin l\u00e0m vi\u1ec7c v\u00e0 li\u00ean h\u1ec7.";}
    @Override protected LineIcons.Name dialogIcon(){return LineIcons.Name.USER;} @Override protected String primaryButtonText(){return "Th\u00eam nh\u00e2n vi\u00ean";} @Override protected int preferredDialogWidth(){return 820;}
    @Override protected FormSchema buildFormSchema(){txtMaNV=createReadonlyField(new DAO_NhanVien().generateNextMaNV());
...
return errors;}
    ...
    @Override protected void onSubmit(NhanVien nv){setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));new SwingWorker<Boolean,Void>(){@Override protected Boolean doInBackground(){return new DAO_NhanVien().insert(nv);}@Override protected void done(){finish(this);}}.execute();/* Handoff: insert preserves saved/onSaved. Risk: generated id is sync now instead of prior worker. */}
    private void finish(SwingWorker<Boolean,Void>w){try{setCursor(Cursor.getDefaultCursor());if(w.get()){saved=true;if(onSaved!=null)onSaved.run();dispose();}else NotionMessageDialog.showMessageDialog(this,"Kh\u00f4ng th\u1ec3 l\u01b0u!","L\u1ed7i",JOptionPane.ERROR_MESSAGE);}catch(Exception ex){NotionMessageDialog.showMessageDialog(this,"L\u1ed7i: "+ex.getMessage(),"L\u1ed7i",JOptionPane.ERROR_MESSAGE);}}
    ...
    public static JComponent buildShadowWrapper(JPanel content) {
        JPanel wrapper = new JPanel(new BorderLayout()) {
            @Override protected void paintComponent(Graphics g) {
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    g2.drawRoundRect(10 - i / 2, 10 - i / 2,
                            getWidth() - 20 + i, getHeight() - 20 + i, 12, 12);
                }
                g2.dispose();
            }
        };
        ...
        return wrapper;
        // Handoff: this static helper is shared by legacy dialogs and the abstract dialog shell.
        // Risk: removing or renaming it breaks many modules that still call ThemNhanVienDialog.buildShadowWrapper.
    }

    protected String passwordLabel(){return "M\u1eadt kh\u1ea9u";} protected boolean passwordRequired(){return true;} protected String passwordHint(){return "M\u1eadt kh\u1ea9u \u0111\u0103ng nh\u1eadp ban \u0111\u1ea7u";} protected void validatePassword(List<ValidationError> e,String p){ if(p.isEmpty()) e.add(new ValidationError("password","Vui l\u00f2ng nh\u1eadp m\u1eadt kh\u1ea9u")); } protected String finalPassword(){return new String(txtPassword.getPassword()).trim();} protected String entityId(){return txtMaNV.getText().trim();}
